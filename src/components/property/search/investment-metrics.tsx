...
const ROI_TOOLTIP =
  "Očekávaná roční návratnost investice vypočítaná z poměru čistého ročního nájmu a celkové pořizovací ceny.";

const CASHFLOW_TOOLTIP =
  "Odhadovaný čistý měsíční zisk z pronájmu po odečtení splátky hypotéky a provozních nákladů.";
...
/** Highlighted investment metrics block for the discovery hub. */
export function InvestmentMetrics({
  ...
  return (
    <FilterSection
      ...
      description="Prémiové metriky pro výnosové vyhledávání"
      className="border-[color-mix(in_srgb,var(--action-accent)_30%,var(--border-default))] bg-[linear-gradient(165deg,color-mix(in_srgb,var(--action-accent)_7%,var(--surface-primary)),var(--surface-primary))]"
      badge={
        <span className="inline-flex items-center gap-1 rounded-full bg-[var(--action-accent)]/12 px-2.5 py-0.5 text-[0.65rem] font-medium text-[var(--action-accent)]">
      ...
      <div className="grid gap-5 sm:grid-cols-2">
        <div className="min-w-0 sm:col-span-1">
          ...
          <input
            ...
            onChange={(e) =>
              onChange({
                roiOd: e.target.value === "" ? undefined : Number(e.target.value),
                cashflowOd,
                rekonstrukceOd,
                rekonstrukceDo,
              })
